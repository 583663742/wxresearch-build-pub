...
static NSString *WXPageHTML(void) {
    ...
    "var html='<div class=mask show id=statsMask><div class=modal-card><div class=modal-head><h3>消息统计</h3><div class=sub>'+fmtRange()+'</div></div>';"
    "for(var i=1;i<rows.length;i++){var r=rows[i];var nm=TYPE_N[r.Type]||('类型'+r.Type);"
    "html+='<div class=stat-line><span class=sl-name>'+nm+'</span><span><span class=sl-cnt>'+r.cnt+'</span><span class=sl-mine>我'+r.mine+' / 对方'+r.theirs+'</span></span></div>';}"
    "html+='<div class=stat-total><span>总计 '+totalCnt+' 条</span><span>我 '+mine+' / 对方 '+theirs+'</span></div>';"
    "html+='<div class=modal-actions>'+'<button onclick=closeStats()>关闭</button>'"
    "+'<button onclick=copyStats('+start+','+end+')>复制</button>'"
    "+'<button class=green onclick=statsToAI('+start+','+end+')>AI分析</button></div>'"
    "+'<div class=modal-read>已阅</div></div></div>';"
    "var old=document.getElementById('statsMask');old.outerHTML=html;}"
    ...
    "html+='<button onclick=setRange(365,this)>近1年</button></div>';"
    ...
    "function setRange(d,btn){aiRange=d;var bs=document.querySelectorAll('#aiRanges button');"
    "for(var i=0;i<bs.length;i++)bs[i].className='';btn.className='on';}"
    ...
    "document.getElementById('content').innerHTML='<div class=ai-loading><span class=spin></span>正在提取聊天记录并交给AI分析…</div>';"
    "post({action:'ai_research',p1:state.db,p2:state.table,p3:state.name,p4:aiRange});}"
